...
_In the horizontal bar charts throughout this report, we include the interquartile range (IQR) to show the middle 50% of respondents’ answers to questions such as salary. One quarter of the respondents has a salary below the displayed range, and one quarter has a salary above the displayed range. The median salary of Php 47,500 will be represented in the box graphs by a dashed blue line._
...
location_summary <-  phackers_in_ph %>% 
  group_by(ncr) %>% 
  summarise(Median=median(salary_numeric), ymin=quantile(salary_numeric, 0.25), ymax=quantile(salary_numeric, 0.75))

location_lineplot <- location_summary %>% ggplot(aes(x=ncr, y=Median, ymin=ymin, ymax=ymax, color=ncr)) + 
  median_hline + 
...
top_industry_phackers %>% group_by(company_industry_f) %>% summarise(median_salary=median(salary_numeric), lower=quantile(salary_numeric, 0.25), upper=quantile(salary_numeric, 0.75)) %>%
  ggplot(aes(x=company_industry_f, color=company_industry_f, y=median_salary, ymin=lower, ymax=upper)) + 
  median_hline + 
...
company_size_summary %>% ggplot(aes(x=company_size_f, y=median_salary, ymin=lower, ymax=upper, color=company_size_f )) + 
  median_hline + 
  geom_linerange(size=5) +
  geom_point(shape='|', size=5, stroke=2, color="white" )  +
  labs(title="Company Size", x="Company Size", y="Salary Range") + 
...
company_age_summary %>% ggplot(aes(x=company_age_f, color=company_age_f, y=median_salary, ymin=lower, ymax=upper)) + 
  median_hline +
  geom_linerange(size=5) +
  geom_point(shape='|', size=5, stroke=2, color="white" )  +
...
team_size_summary %>% ggplot(aes(x=team_size_f, color=team_size_f, y=y, ymin=lower, ymax=upper)) +
  median_hline +
  geom_linerange(size=5) +
  geom_point(shape='|', size=5, stroke=2, color="white" )  +
  labs(title="Team Size", x="Team Size", y="Salary Range") + 
...
project_size_summary %>% ggplot(aes(x=project_size_f, color=project_size_f, y=y, ymin=lower, ymax=upper)) +
  median_hline +
  geom_linerange(size=5) +
  geom_point(shape='|', size=5, stroke=2, color="white" )  +
  labs(title="Project Size", x="Project Size", y="Salary Range") + 
...
freelance_boxplot <- phackers %>% ggplot(aes(x=employment_f, y=salary_numeric, fill=employment_f)) +
  median_hline + 
  geom_boxplot() +
  labs(title="Employment", x="Employed or Freelance", y="Salary Range") +
  guides(fill="none")

freelance_salary_summary %>% ggplot(aes(x=employment_f, color=employment_f, y=median_salary, ymin=lower, ymax=upper)) +
  median_hline +
  geom_linerange(size=10) +
  geom_point(shape='|', size=10, stroke=2, color="white" )  +
