...
```{r, echo=FALSE}
freelance_salary_summary <- phackers %>% group_by(employment_f) %>% summarise(median(salary_numeric), n=n(), percent=round(100 * n()/sum(count(phackers)),2))

phackers %>% ggplot(aes(x=employment_f, y=salary_numeric, fill=employment_f)) +
  geom_boxplot() +
  geom_hline(yintercept = 47500, linetype="dashed", color="blue") +
