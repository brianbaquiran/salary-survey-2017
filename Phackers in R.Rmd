...
```
```{r}
phackers %>% filter(!outlier) %>% ggplot(aes(age, fill=gender)) + geom_bar(stat="count") + 
...
phackers %>% filter(!outlier) %>% ggplot(aes(y=sal_num, x=exp_num, color=company_size_f)) + geom_jitter() + facet_grid(~company_age_f)
```
```{r}
